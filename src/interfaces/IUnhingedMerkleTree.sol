 ...
 * @dev The Unhinged Merkle Tree is designed for efficient cross-chain proofs where each chain only needs to process what's relevant to it
 */
interface IUnhingedMerkleTree {
     ...
     */
    function extractCounts(bytes32 counts) external pure returns (uint120 subtreeProofCount, uint120 followingHashesCount, bool hasPreHash);
     ...
     */
    function packCounts(uint120 subtreeProofCount, uint120 followingHashesCount, bool hasPreHash) external pure returns (bytes32 counts);
