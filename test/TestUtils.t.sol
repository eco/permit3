...
import { Permit3TestUtils, MockToken } from "./utils/TestUtils.sol";
...
contract TestUtilsTest is Test {
    ...
    function test_hashChainPermits() public {
        ...
        for (uint256 i = 0; i < permits.length; i++) {
            permitHashes[i] = keccak256(
                abi.encode(
                    permits[i].modeOrExpiration,
                    permits[i].token,
                    permits[i].account,
                    permits[i].amountDelta
                )
            );
        }
    ...
    function test_createTransferPermit() public {
        ...
        IPermit3.ChainPermits memory transferPermit = Permit3TestUtils.createTransferPermit(
            testToken,
            testRecipient,
            testAmount
        );
