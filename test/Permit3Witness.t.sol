...
contract Permit3WitnessTest is Test {
    ...
    function test_permitWitnessTransferFrom() public {
        ...
        bytes memory signature = _signWitnessPermit(chainPermits, deadline, timestamp, SALT, WITNESS, WITNESS_TYPE_STRING);
    ...
    function test_permitWitnessTransferFromInvalidSignature() public {
        ...
        vars.typeHash = keccak256(
            abi.encodePacked(
                permit3.PERMIT_WITNESS_TYPEHASH_STUB(),
                WITNESS_TYPE_STRING
            )
        );
    ...
    function test_permitWitnessTransferFromCrossChain() public {
        ...
        bytes memory signature = _signWitnessUnhingedPermit(proof, deadline, timestamp, SALT, WITNESS, WITNESS_TYPE_STRING);
    ...
    function _createUnhingedProof() internal view returns (IPermit3.UnhingedPermitProof memory) {
        ...
        return IPermit3.UnhingedPermitProof({
    ...
    function _signWitnessUnhingedPermit(
        ...
        for (uint256 i = 0; i < vars.followingHashesCount; i++) {
            vars.unhingedRoot = keccak256(abi.encodePacked(
                vars.unhingedRoot,
                proof.unhingedProof.nodes[vars.nodeIndex + i]
            ));
        }
