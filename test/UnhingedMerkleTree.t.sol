...
contract UnhingedMerkleTreeTest is Test {
    ...
    function test_packAndExtractCounts() public pure {
        ...
        (uint120 extractedSubtreeCount, uint120 extractedFollowingCount, bool extractedHasPreHash) =
            UnhingedMerkleTree.extractCounts(packed);
    ...
    function test_invalidProofWithInsufficientNodes() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IUnhingedMerkleTree.InvalidNodeArrayLength.selector, 3, 2));
    ...
    function test_createUnhingedRoot() public view {
        ...
        bytes32 expected = keccak256(
            abi.encodePacked(keccak256(abi.encodePacked(multipleHashes[0], multipleHashes[1])), multipleHashes[2])
        );
    ...
    function test_hasPreHashGasBenchmarkComprehensive() public {
        ...
        // Scenario 1: Simple proof with empty subtree proof
        {
            ...
            IUnhingedMerkleTree.UnhingedProof memory withoutPreHash =
                IUnhingedMerkleTree.UnhingedProof({ nodes: new bytes32[](0), counts: tester.packCounts(0, 0, false) });
        ...
        // Scenario 2: Small subtree proof (1 node)
        {
            ...
            IUnhingedMerkleTree.UnhingedProof memory withPreHash =
                IUnhingedMerkleTree.UnhingedProof({ nodes: nodesWithPreHash, counts: tester.packCounts(1, 0, true) });
            ...
            // Without preHash (optimized)
            IUnhingedMerkleTree.UnhingedProof memory withoutPreHash =
                IUnhingedMerkleTree.UnhingedProof({ nodes: subtreeProof, counts: tester.packCounts(1, 0, false) });
        ...
        // Scenario 3: With following hashes
        {
            ...
            IUnhingedMerkleTree.UnhingedProof memory withPreHash =
                IUnhingedMerkleTree.UnhingedProof({ nodes: nodesWithPreHash, counts: tester.packCounts(0, 2, true) });
            ...
            // Without preHash (optimized)
            IUnhingedMerkleTree.UnhingedProof memory withoutPreHash =
                IUnhingedMerkleTree.UnhingedProof({ nodes: followingHashes, counts: tester.packCounts(0, 2, false) });
    ...
    function test_inconsistentPreHashFlag() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IUnhingedMerkleTree.InconsistentPreHashFlag.selector, true, bytes32(0)));
