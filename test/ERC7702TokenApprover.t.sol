...
contract MockERC20 {
    ...
    function approve(address spender, uint256 amount) external returns (bool) {
        if (shouldFailApproval) return false;
        allowance[msg.sender][spender] = amount;
        return true;
    }
    
    function setShouldFailApproval(bool _shouldFail) external {
...
contract MockEOA {
    ...
    constructor(ERC7702TokenApprover _approver) {
        approver = _approver;
    }
    
    // Simulates what would happen when EOA delegatecalls to ERC7702TokenApprover
    function simulateERC7702Approval(address[] calldata tokens) external {
...
contract ERC7702TokenApproverTest is Test {
    ...
    function testFuzz_Approve(uint8 tokenCount) public {
