...
contract TestBase is Test {
    ...
    function _hashChainPermits(IPermit3.ChainPermits memory chainPermits) internal view returns (bytes32) {
    ...
    function _signUnhingedPermit(
        ...
        for (uint256 i = 0; i < params.followingHashesCount; i++) {
            params.unhingedRoot = keccak256(abi.encodePacked(
                params.unhingedRoot,
                proof.unhingedProof.nodes[params.nodeIndex + i]
            ));
        }
    ...
    function exposed_hashTypedDataV4(bytes32 structHash) internal view returns (bytes32) {
