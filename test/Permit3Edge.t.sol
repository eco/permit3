...
contract Permit3EdgeTest is Test {
    ...
    function test_permitBatchWithEmptyArray() public {
        ...
        inputs.chainPermits = IPermit3.ChainPermits({
    ...
    function test_getUnhingedWitnessTypeHash() public {
        ...
        vars.unhingedProof = IPermit3.UnhingedPermitProof({
            permits: inputs.chainPermits,
            unhingedProof: vars.merkleProof
        });
        ...
        vars.typeHash = keccak256(
            abi.encodePacked(
                permit3.PERMIT_UNHINGED_WITNESS_TYPEHASH_STUB(),
                params.witnessTypeString
            )
        );
    ...
    function test_verifyUnhingedProofInvalid() public {
        ...
        IPermit3.UnhingedPermitProof memory unhingedProof = IPermit3.UnhingedPermitProof({
            permits: inputs.chainPermits,
            unhingedProof: invalidProof
        });
    ...
    function test_transactionOrderProtection() public {
        ...
        newerParams.timestamp = uint48(11000); // Newer timestamp
        ...
        olderInputs.chainPermits = IPermit3.ChainPermits({
            chainId: block.chainid,
            permits: olderInputs.permits
        });
        ...
        newerInputs.chainPermits = IPermit3.ChainPermits({
            chainId: block.chainid,
            permits: newerInputs.permits
        });
    ...
    function test_calculateUnhingedRootInvalidLength() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(IUnhingedMerkleTree.InvalidNodeArrayLength.selector, 4, 2)
        );
    ...
    function test_typehashStubs() public {
        ...
        assertEq(permitStub, "PermitWitnessTransferFrom(ChainPermits permitted,address spender,bytes32 salt,uint256 deadline,uint48 timestamp,");
        assertEq(batchStub, "PermitBatchWitnessTransferFrom(ChainPermits[] permitted,address spender,bytes32 salt,uint256 deadline,uint48 timestamp,");
        assertEq(unhingedStub, "PermitUnhingedWitnessTransferFrom(bytes32 unhingedRoot,address owner,bytes32 salt,uint256 deadline,uint48 timestamp,");
    }
    ...
    function test_lockAndDecrease() public {
        ...
        lockInputs.chainPermits = IPermit3.ChainPermits({
            chainId: block.chainid,
            permits: lockInputs.permits
        });
        ...
        decreaseInputs.chainPermits = IPermit3.ChainPermits({
            chainId: block.chainid,
            permits: decreaseInputs.permits
        });
    ...
    function test_lockAndUnlockAllowance() public {
        ...
        unlockInputs.chainPermits = IPermit3.ChainPermits({
            chainId: block.chainid,
            permits: unlockInputs.permits
        });
    ...
    function test_emptyProofHandling() public {
        ...
        IUnhingedMerkleTree.UnhingedProof memory emptyProof = IUnhingedMerkleTree.UnhingedProof({
            nodes: nodes,
            counts: counts
        });
